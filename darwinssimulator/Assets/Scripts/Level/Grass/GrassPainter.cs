...
public class GrassPainter : MonoBehaviour
{
    ...
    void RebuildMesh()
    {
     ...
     public void PlaceGrass(Vector3 position) // non fonctionnel
    {
        RaycastHit hit;
        
        // Raycast pour obtenir les informations du point de placement
        if (Physics.Raycast(position + Vector3.up * 100f, Vector3.down, out hit, 200f, hitMask.value))
        {
            // Vérifier si le point est valide en fonction du masque de peinture
            if ((paintMask.value & (1 << hit.transform.gameObject.layer)) > 0)
            {
                Vector3 grassPosition = hit.point - transform.position;
                positions.Add(grassPosition);
                indicies.Add(i);
                length.Add(new Vector2(sizeWidth, sizeLength));
                colors.Add(new Color(AdjustedColor.r + (Random.Range(0, 1.0f) * rangeR), AdjustedColor.g + (Random.Range(0, 1.0f) * rangeG), AdjustedColor.b + (Random.Range(0, 1.0f) * rangeB), 1));
